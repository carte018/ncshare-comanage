      ...
      if($supportedAttributes[$oc]['objectclass']['required']
        ...
        foreach(array_keys($supportedAttributes[$oc]['attributes']) as $attr) {
          ...
          if($supportedAttributes[$oc]['attributes'][$attr]['required']
			...
			// RGC -- try using groupname type of UID
			$nameIdentifier = Hash::extract($provisioningData['Identifier'],'{n}[type=uid]');
			if ($nameIdentifier) {
			  $attributes[$lattr] = $nameIdentifier[0]['identifier'];
			}
		      }
			...
			// RGC -- force with gidNumber type
			$gidIdentifier = Hash::extract($provisioningData['Identifier'],'{n}[type=gidNumber]');
			if ($gidIdentifier) {
				$attributes[$lattr] = $gidIdentifier[0]['identifier'];
			}
		    }
			...
			$members = $this->CoLdapProvisionerDn
					->CoGroup
					->CoGroupMember
					->mapCoGroupMembersToIdentifiers($groupMembers, 'uid');
			if (!empty($members)) {
				$attributes[$attr] = $members;
			} elseif($modify) {
				$attributes[$attr] = array();
        ...
        foreach($cfg['attributes'] as $attr => $acfg) {
          if($acfg['required']) {
            ...
            if(!array_filter($attributes, 
                             ...
                             ARRAY_FILTER_USE_KEY)) {
	      /*
	       * RGC - In the event that a new CoPerson is being provisioned AND the
	       * UnixCluster plugin is being used AND groups are being managed as 
	       * posixGroup objects in the LDAP, there is an order of operations 
	       * problem that appears to prevent posixGroup objects from being 
	       * filled out properly (probably due to UnixClusterGroup information not 
	       * being committed to the database before group provisioning is attempted).
	       * This results in LDAP provisioning failures, which we address elsewhere
	       * by re-provisioning affected groups at the end of the enrollment process.
	       * It also, however, results in the array_search() call below erroring out 
	       * as a result of the $attributes array having no 'objectclass' member for 
	       * the provisioning candidate.  That error interrupts the enrollment flow,
	       * so to avoid the condition, we skip the (impossible) attempt to remove
	       * valueless attributes from the NULL objectclass array. 
	       */
  ...
  public function provision($coProvisioningTargetData, $op, $provisioningData) {
    ...
    if($modify) {
      ...
      if(!@ldap_mod_replace($cxn, $dns['newdn'], $attributes)) {
        error_reporting($current_error_reporting);
	if(ldap_errno($cxn) == 0x43 || ldap_errno($cxn) == 0x40) { /*LDAP_NOT_ALLOWED_ON_RDN or LDAP_NAMING_VIOLATION*/
		$carrier = $attributes['cn'];
		$carray = array();
		$carray[] = $carrier;
		$ld = rtrim(str_replace($coProvisioningTargetData['CoLdapProvisionerTarget']['group_basedn'], "", $dns['newdn']), " ,");
		$rd = str_replace("cn=","",$ld);
		$carray[] = $rd;
		$attributes['cn'] = $carray;
		if(!@ldap_mod_replace($cxn,$dns['newdn'],$attributes)) {
			error_log("RGC -- group modification failed, despite trying cn extension");
		}
	}
