...
class CoPetitionsController extends StandardController {
  ...
  protected function execute_provision($id) {
    ...
    if($status == PetitionStatusEnum::Finalized) {
      ...
      /*
       * RGC - Provisioning of posixGroups as UnixClusterGroups during enrollment flows
       * appears to be attempted before UnixClusterGroup information is committed to the 
       * database, resulting in hard errors during LDAP provisioning.  Originally, this 
       * terminated the enrollment flow with a PHP error (due to referencing an unretrievable
       * attribute from the database).  Here, we re-run group provisioining as part of the 
       * final provisioning process for enrollment *after* everything has been committed.
       * Only groups the CoPerson is the first (or only) member of will be reprovisioned,
       * so that usually only the one user-specific group is run through provisioning.  
       * No harm should other groups get reprovisioned, save for a little extra processing.
       */
      $args = array();
      $gs = $this->CoPetition->Co->CoGroup->find("all",NULL);

      $this->CoPetition->Co->CoGroup->Behaviors->load('Provisioner');
      foreach ($gs as $gi) {
	if ($gi['CoGroupMember'][0]['co_person_id'] == $coPersonId) { 
		$gid = $gi['CoGroup']['id'];
		$this->CoPetition->Co->CoGroup->manualProvision(null, null, $gid, ProvisioningActionEnum::CoGroupUpdated);
	}
      }
      /* ! RGC */
  ...
  protected function execute_redirectOnConfirm($id) {
   ...
   /* RGC */
    /* Force logout here, since otherwise the state of the user remains "in progress" in the browser */
...
// XXX don't want to do this where more than 1 org identity can be linked
  ...
  protected function execute_sendConfirmation($id) {
    ...
    /*RGC */ /*$this->CoPetition->sendConfirmation($id, $this->Session->read('Auth.User.co_person_id'));*/
    /* RGC */ $frag = $this->CoPetition->sendFakeConfirmation($id, $this->Session->read('Auth.User.co_person_id'));
    ...
    if($ef["CoEnrollmentFlow"]["email_verification_mode"] === VerificationModeEnum::SkipIfVerified
      ...
      /*RGC*/ /*$this->redirect($this->generateDoneRedirect('sendConfirmation', $id));*/
      /*RGC*/ $this->redirect('/co_invites/reply/' . $frag);
    } else {
