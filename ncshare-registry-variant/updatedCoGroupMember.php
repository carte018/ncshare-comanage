...
//  public $order = array("co_person_id");
  ...
  public function syncNestedMembership($targetGroup, $coGroupNestingId, $coPersonId) {
    ...
    if(!$isCurrent && $shouldBe) {
      ...
      //
      // Specific mod to group membership behavior for nested groups for NCShare.
      // In the NCShare case, we have two unique conceits:
      //
      // If a group whose name matches '^(.*)_h200' exists and has as a nested member
      // a group whose name matches '^$1_contacts', the members of the contacts group
      // should be embedded as owners, rather than members, of the outer group. This is 
      // to allow automation of management of h200 authorization groups in COManage by 
      // institutional contacts for schools participating in NCShare. 
      // 
      // If any group has the group ncshare_support nested within it, members of
      // ncshare_support should nest into that group as owners, rather than members.  This 
      // is to afford NCShare support group members the ability to manage groups without 
      // having to individually maintain owner entries in those groups for each of them.
      //
      // We start by acquiring the names of the relevant groups -- nestedName becomes the 
      // name of the interior group and targetName becomes the name of the target group (the
      // outer group for the nesting).
      //
      $nargs = array();
      $nargs['CoGroupNesting.id'] = $coGroupNestingId;
      $nargs['contain'] = array('CoGroup');
      $ncoGroupNestings = $this->CoGroup->CoGroupNesting->find('all', $args);
      // Given the list of all nestings in this nesting id, we need to now find the last one
      // that contains the user being embedded.
      foreach ($ncoGroupNestings as $ngn) {
	if($this->isMember($ngn['CoGroupNesting']['co_group_id'], $coPersonId)) {
		$ncoGroupNesting = $ngn;
        }
      }
      $nestedName = $ncoGroupNesting['CoGroup']['name'];
      $targetName = $targetGroup['name'];

      $this->log('Processing embed sync for '.$nestedName.' into '.$targetName);
      //
      // Now we perform the comparisons and choose whether $data['owner'] should be 1 or 0
      //
      if (preg_match('/(.*)_h200/',$targetName,$parts)) {
	// This is a nesting into an _h200 group -- check if this is the _contacts group nesting
	if ($nestedName === $parts[1].'_contacts' || $nestedName === 'ncshare_support') {
		// This nesting foo_contacts into foo_h200 -- make the nested members owners
		$this->log("Setting owner level embed for ".$nestedName." in ".$targetName);
		$data['CoGroupMember']['owner'] = 1; 
	} else {
		$this->log("Setting member level embed for ".$nestedName." in ".$targetName);
		$data['CoGroupMember']['owner'] = 0;  // other embeddings are members only
	}
      }  else if ($nestedName === 'ncshare_support') {
		// In other cases, ncshare_support become owners when embedded
		$this->log("Setting owner embed for ".$nestedName." in ".$targetName);
		$data['CoGroupMember']['owner'] = 1;
      } else {
	// All other cases are member only
	$this->log("Setting member embed for ".$nestedName." in ".$targetName);
	$data['CoGroupMember']['owner'] = 0;
      }
