...
class CoInvite extends AppModel {
  ...
  // Add an id carrier that gets set when needed
  public $idcarrier = '';
  ...
  public function getInvitationFragment($coPersonId,
					$orgIdentityID,
					$toEmail,
					$emailAddressID = null) {
	try {
		$this->deleteAll(array('CoInvite.co_person_id' => $coPersonId,
					'CoInvite.mail' => $toEmail), false, true);
	} catch (Exception $e) {
		throw new RuntimeException($e->getMessage());
	}
   	$invite = array();
    	...
    	$invite['CoInvite']['skip_invite'] = false;
    	$invite['CoInvite']['expires'] = date('Y-m-d H:i:s', strtotime('+ 1440 minutes'));
	if ($emailAddressID) {
	...
	$idcarrier = $this->id;
        ...
        $retv = array(
		"frag" => $invite['CoInvite']['invitation'],
		"otherid" => $this->id,
	);

    	if($this->save()) {
      	//	return($invite['CoInvite']['invitation']);
		$retv["otherid"] = $this->id;
		return($retv);
	}
}
